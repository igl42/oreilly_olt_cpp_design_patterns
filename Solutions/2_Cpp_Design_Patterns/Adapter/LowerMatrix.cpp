...
class LowerMatrix
   : public DenseMatrix< LowerMatrix<MatrixType> >
{
 public:
   ...
   explicit LowerMatrix( size_t size )
      : matrix_( size, size, value_type{} )
   {}

   size_t rows() const { return matrix_.rows(); }
   size_t cols() const { return matrix_.cols(); }

   value_type const* data() const { return matrix_.data(); }
   value_type const* data( size_t i ) const { return matrix_.data(i); }
...
int main()
{
   ...
   try {
      L(0,1) = 7;  // Runtime error

      std::cerr << "Access succeeded, upper element was modified :-(\n";
   }
